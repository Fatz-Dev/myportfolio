...
html = html.replace('<!-- Injected via JS -->', expSkeleton); // Replaces first one (Experience)
...
html = html.replace('<!-- Injected via JS -->', certSkeleton); // Replaces second one (Certificates)
